			...
			if($this->entries_per_month[$j] > 0){
				...
				for($i=count($this->entry_list)-1;$i >= 0;$i--){
					...
					if($pos === false){
						...
						$url_title = str_replace(" ", "-", $this->entry_list[$i]->get_title());
						?>
							<li><a href="http://localhost/blog/<?php echo $url_title; ?>/" title="<?php echo $this->entry_list[$i]->get_title(); ?>"><?php echo $this->entry_list[$i]->get_title(); ?></a>
							<b><?php echo $this->entry_list[$i]->get_date(); ?></b></li>
						<?php
					}
				}
				?>
